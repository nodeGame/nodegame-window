...
function GameWindow() {
	...
	// and an object as a value with the fields 'contents' (a string describing the
	// innerHTML or null if not cached) and optionally 'cacheOnClose'
	// (a bool telling whether to cache the frame when it is replaced by a new one).
	this.cache = {};
		...
		iframe.onload = (function(uri, thisIframe) {
			return function() {
				...
				var frameDocumentElement =
					...
					.documentElement;
				that.cache[uri] = { contents: frameDocumentElement.innerHTML, cacheOnClose: false };
...
GameWindow.prototype.load = GameWindow.prototype.loadFrame = function (uri, func, opts) {
	...
	var loadCache = false;
	var storeCacheNow = false;
	var storeCacheLater = false;
	...
	if (opts) {
		...
		if (opts.cache) {
			if (opts.cache.loadMode === 'cache') loadCache = true;

			if (opts.cache.storeMode === 'onLoad') storeCacheNow = true;
			else if (opts.cache.storeMode === 'onClose') storeCacheLater = true;
		}
	}
	...
	// Store cacheOnClose flag:
	if(!(uri in this.cache)) this.cache[uri] = { contents: null };
	...
	iframe.onload = function() {
		...
		if (loadCache && (uri in that.cache) && that.cache[uri].contents !== null) {
		...
		if (storeCacheNow) {
			...
			that.cache[uri] = { contents: frameDocumentElement.innerHTML, cacheOnClose: storeCacheLater };
	...
	// Cache lookup:
	if (loadCache && (uri in this.cache) && this.cache[uri].contents !== null) {
